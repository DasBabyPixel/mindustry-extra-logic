...
public class ExtraLogicBlock extends Block {
	...
	public ExtraLogicBlock(String name) {
		...
		config(Integer.class, (ExtraLogicBuilding entity, Integer pos) -> {
			if (!accessible())
				return;
	...
	public static String getLinkName(Block block) {
		...
		if (name.contains("-")) {
			...
			if (split.length >= 2
					&& (split[split.length - 1].equals("large") || Strings.canParseFloat(split[split.length - 1]))) {
	...
	public class ExtraLogicBuilding extends Building implements Ranged {
		...
		public ExtraLExecutor executor = new ExtraLExecutor();
...
//					throw new IOException("Malformed logic data! Length: " + bytelen);
		...
		public void updateCode(String str, boolean keep, Cons<ExtraLAssembler> assemble) {
			if (str != null) {
				code = str;

				try {
					// create assembler to store extra variables
					ExtraLAssembler asm = ExtraLAssembler.assemble(str, privileged);
		...
		public boolean validLink(Building other) {
			return other != null && other.isValid()
					&& (privileged || (!other.block.privileged && other.team == team
